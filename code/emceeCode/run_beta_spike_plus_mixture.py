...
from posteriors import betaSpikePlusMixture
...
# Model 
model = "component_spin_betaSpikePlusMixture"
...
if len(old_chains)==0:
    ...
    initial_fracs = draw_initial_walkers_uniform(nWalkers, (0.05, 0.1))
    ...
    initial_walkers = np.transpose(
        [initial_mu_chis, initial_sigma_chis, initial_MFs, initial_sigma_costs, initial_fracs, initial_sigma_ss, initial_Bqs]
    )
...
if nSteps>0: # if the run hasn't already finished
    ...
    sampler = mc.EnsembleSampler(
        nWalkers,
        dim,
        betaSpikePlusMixture, # model in posteriors.py
