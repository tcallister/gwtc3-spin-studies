...
def betaSpikePlusTruncatedMixture_Galaudage(c,sampleDict,injectionDict,priorDict): 
    
    """
    Implementation of the BetaSpike+TruncatedMixture model more similar to that in Galaudage+: 
    similar to betaSpikePlusTruncatedMixture BUT assumes 1. both spin magnitudes are either in the 
    spike or bulk, not allowing for the one in each, and 2. both spin tilts are either in the 
    isotropic or aligned distribution, not one in each.
    ...
    # If the sample falls inside our prior range, continue
        ...
        # Detected spins
        p_chi1_spike_det = calculate_Gaussian_1D(chi1_det, 0, sigma_spike, 0, 1)
        p_chi2_spike_det = calculate_Gaussian_1D(chi2_det, 0, sigma_spike, 0, 1)
        p_chi1_bulk_det = betaDistribution(chi1_det, a, b)
        p_chi2_bulk_det = betaDistribution(chi2_det, a, b)
        p_chi_det = frac_in_spike*p_chi1_spike_det*p_chi2_spike_det + (1-frac_in_spike)*p_chi1_bulk_det*p_chi2_bulk_det
        
        p_cost1_iso_det = 1/(1-cost_min)
        p_cost2_iso_det = 1/(1-cost_min)
        p_cost1_aligned_det = calculate_Gaussian_1D(cost1_det, 1, sigma_cost, cost_min, 1)
        p_cost2_aligned_det = calculate_Gaussian_1D(cost2_det, 1, sigma_cost, cost_min, 1)
        p_cost_det = MF_cost*p_cost1_aligned_det*p_cost2_aligned_det + (1-MF_cost)*p_cost1_iso_det*p_cost2_iso_det
       
        pdet_spins = p_chi_det*p_cost_det
        ...
        for event in sampleDict:
            ...
            # Evaluate model at the locations of samples for this event
            p_chi1_spike = calculate_Gaussian_1D(chi1_samples, 0, sigma_spike, 0, 1)
            p_chi2_spike = calculate_Gaussian_1D(chi2_samples, 0, sigma_spike, 0, 1)
            p_chi1_bulk = betaDistribution(chi1_samples, a, b)
            p_chi2_bulk = betaDistribution(chi2_samples, a, b)
            p_chi = frac_in_spike*p_chi1_spike*p_chi2_spike + (1-frac_in_spike)*p_chi1_bulk*p_chi2_bulk

            p_cost1_iso = 1/(1-cost_min)
            p_cost2_iso = 1/(1-cost_min)
            p_cost1_aligned = calculate_Gaussian_1D(cost1_samples, 1, sigma_cost, cost_min, 1)
            p_cost2_aligned = calculate_Gaussian_1D(cost2_samples, 1, sigma_cost, cost_min, 1)
            p_cost = MF_cost*p_cost1_aligned*p_cost2_aligned + (1-MF_cost)*p_cost1_iso*p_cost2_iso
