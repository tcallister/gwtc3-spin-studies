...
from posteriors import betaPlusMixture
...
if len(old_chains)==0:
    ...
    initial_walkers = np.transpose(
        [initial_mu_chis, initial_sigma_chis, initial_MFs, initial_sigma_costs, initial_Bqs]
    )
...
if nSteps>0: # if the run hasn't already finished
    ...
    sampler = mc.EnsembleSampler(
        nWalkers,
        dim,
        betaPlusMixture, # model in posteriors.py
