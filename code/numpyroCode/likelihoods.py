...
def gaussianSpike(sampleDict,injectionDict,mMin,sig_eps):

    """
    Implementation of a Gaussian and zero-spin spike effective spin distribution for inference within `numpyro`.
    As this implementation makes use of KDEs over posterior samples and found injections, the "spike width"
    parameter `sig_eps` can be zero.
...
def gaussianSpike_MonteCarloAvg(sampleDict,injectionDict,mMin,sig_eps):
    ...
    """
    Implementation of a Gaussian and zero-spin spike effective spin distribution for inference within `numpyro`.
    Relies on standard Monte Carlo averaging of posterior samples and found injections, hence the "spike width"
    parameter `sig_eps` must be non-zero.
